...
function currentStatus(rec: DailyTimeRecord): { label: string; color: string; bg: string } {
  ...
  if (actions.has('breakStart') && !actions.has('breakEnd')) return { label: 'In Pause',        color: '#92400E', bg: '#FEF3C7' };
            ...
            corrections.length === 0
              ...
              : corrections.map(c => (
                  <View key={c.id} style={styles.card}>
                    ...
                    <Text style={styles.corrDate}>{c.date} · {ACTION_LABELS[c.missingAction] ?? c.missingAction}</Text>
